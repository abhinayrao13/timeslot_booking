class Event < ApplicationRecord
    ...
    def self.get_events_data
      all.collect{|a| {
        title: a.reason,
        start: "#{a.date.strftime('%Y-%m-%d')}T#{a.start_time.strftime('%H:%M:%S')}",
      end: "#{a.date.strftime('%Y-%m-%d')}T#{a.start_time.strftime('%H:%M:%S')}"
      }}.to_json.html_safe
    end
    ...
    def validate_datetime?
      if date.present? && start_time.present? && end_time.present?
        if (date.to_formatted_s(:db) rescue ArgumentError) == ArgumentError
          ...
          # Event.where(date: date).each do |a|
          #   if start_time.between?(a.start_time, a.end_time) || end_time.between?(a.start_time, a.end_time)
          #     errors.add(:error, "Event exits, Please select different time")
          #     break
          #   end
          # end
        end
